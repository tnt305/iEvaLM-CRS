...
class KGForUniCRS:
    def __init__(self, kg, debug=False):
        ...
        with open(
            os.path.join(self.dataset_dir, "kg.json"), "r", encoding="utf-8"
        ) as f:
            self.kg = json.load(f)
        with open(
            os.path.join(self.dataset_dir, "entity2id.json"),
            "r",
            encoding="utf-8",
        ) as f:
            self.entity2id = json.load(f)
        with open(
            os.path.join(self.dataset_dir, "relation2id.json"),
            "r",
            encoding="utf-8",
        ) as f:
            self.relation2id = json.load(f)
        with open(
            os.path.join(self.dataset_dir, "item_ids.json"),
            "r",
            encoding="utf-8",
        ) as f:
